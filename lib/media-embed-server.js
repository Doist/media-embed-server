...
(function() {
  var Memcached, app, async, attachThumbnail, attachTitle, attachType, attachUrl, express, handleAsyncCall, http, imagesize, media_parser, mime, parse, partial, program, usage, webpage_info;
          ...
          if (cached_data) {
            ...
            return media_parser.parse(url, function(parsed_obj) {
              ...
              if (parsed_obj) {
                ...
                update_cache = true;
                return handleAsyncCall(async_cb, cache_key, update_cache, result, timeout);
              } else {
  ...
  handleAsyncCall = function(async_cb, cache_key, update_cache, result, timeout) {
    var callback, request, thumbnail, turl;
    callback = function() {
      if (update_cache) {
    ...
    if (thumbnail && thumbnail.url && (!thumbnail.width || !thumbnail.height)) {
      ...
      return request = http.get(turl, function(response) {
        return imagesize(response, function(err, image_result) {
          if (image_result && image_result.width && image_result.height) {
  ...
  attachType = function(version, result, url) {
    return result.type = mime.lookup(url) || 'application/octet-stream';
  };
